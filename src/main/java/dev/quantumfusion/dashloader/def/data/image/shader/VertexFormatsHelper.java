...
public class VertexFormatsHelper {
	...
	public static Value getEnum(VertexFormat format) {
		...
		if (format == VertexFormats.BLIT_SCREEN) return Value.BLIT_SCREEN;
		if (format == VertexFormats.POSITION_COLOR_TEXTURE_LIGHT_NORMAL) return Value.POSITION_COLOR_TEXTURE_LIGHT_NORMAL;
		if (format == VertexFormats.POSITION_COLOR_TEXTURE_OVERLAY_LIGHT_NORMAL) return Value.POSITION_COLOR_TEXTURE_OVERLAY_LIGHT_NORMAL;
		if (format == VertexFormats.POSITION_TEXTURE_COLOR_LIGHT) return Value.POSITION_TEXTURE_COLOR_LIGHT;
		if (format == VertexFormats.POSITION) return Value.POSITION;
		if (format == VertexFormats.POSITION_COLOR) return Value.POSITION_COLOR;
		if (format == VertexFormats.LINES) return Value.LINES;
		if (format == VertexFormats.POSITION_COLOR_LIGHT) return Value.POSITION_COLOR_LIGHT;
		if (format == VertexFormats.POSITION_TEXTURE) return Value.POSITION_TEXTURE;
		if (format == VertexFormats.POSITION_COLOR_TEXTURE) return Value.POSITION_COLOR_TEXTURE;
		if (format == VertexFormats.POSITION_TEXTURE_COLOR) return Value.POSITION_TEXTURE_COLOR;
