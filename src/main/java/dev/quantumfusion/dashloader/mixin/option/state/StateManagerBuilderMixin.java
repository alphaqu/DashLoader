...
public abstract class StateManagerBuilderMixin<O, S extends State<O, S>> {
	...
	public StateManager<O, S> build(Function<O, S> ownerToStateFunction, StateManager.Factory<O, S> factory) {
		ImmutableSortedMap<String, Property<?>> properties = ImmutableSortedMap.copyOf(this.namedProperties);
		Supplier<S> supplier = () -> ownerToStateFunction.apply(this.owner);
		...
		final Property[] propertiesRaw = this.namedProperties.values().toArray(Property[]::new);
		...
		for (var entry : StateMath.generateCombinations2Mod(propertiesRaw, propertyValues)) {
			...
			S state = factory.create(this.owner, entries, finalMapCodec);
