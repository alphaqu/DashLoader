...
public class StagedWriteChunk<R, D extends Dashable<R>> extends AbstractWriteChunk<R, D> {
	private final Object2ObjectLinkedOpenHashMap<Class<?>, List<IndexedArrayMapTask.Entry<D>>> dashList = new Object2ObjectLinkedOpenHashMap<>();
	...
	public int add(R raw) {
		...
		this.dashList.computeIfAbsent(created.getClass(), c -> new ArrayList<>()) // just in case the fallback returns an unknown model
				.add(new IndexedArrayMapTask.Entry<>(created, this.currentPos));
	...
	public DataChunk<R, D> exportData() {
		...
		IndexedArrayMapTask.Entry<D>[][] out = new IndexedArrayMapTask.Entry[this.dashList.size()][];

		int i = 0;
		int size = 0;
		for (List<IndexedArrayMapTask.Entry<D>> value : this.dashList.values()) {
			//noinspection unchecked
			out[i++] = value.toArray(IndexedArrayMapTask.Entry[]::new);
			size += value.size();
		}
